...
async function runAllTests() {
    ...
    const compRes = await (0, db_1.query)('INSERT INTO companies (name) VALUES ($1) RETURNING company_id', [compName]);
    const companyId = compRes.rows[0].company_id;
    await assert(!!companyId, 'Company onboarding created company_id');
    const { loginId: adminLoginId, serialNo: adminSerial } = await (0, loginId_1.generateLoginId)(companyId, compName, 'Marc', 'Admin', 2026);
    await assert(adminLoginId === 'NOMAAD20260001', `Admin login ID generation matches prefix rules. Got: ${adminLoginId}`);
    const adminPassHash = await bcryptjs_1.default.hash('AdminPass123!', 12);
    const adminRes = await (0, db_1.query)(`INSERT INTO users (company_id, login_id, email, password_hash, role, must_change_password)
     VALUES ($1, $2, 'admin@nmitodoo.com', $3, 'admin', FALSE) RETURNING user_id`, [companyId, adminLoginId, adminPassHash]);
    ...
    const { loginId: emp1LoginId, serialNo: emp1Serial } = await (0, loginId_1.generateLoginId)(companyId, compName, 'John', 'Doe', 2026);
    ...
    const { loginId: emp2LoginId, serialNo: emp2Serial } = await (0, loginId_1.generateLoginId)(companyId, compName, 'Jane', 'Smith', 2027);
    ...
    const emp1Res = await (0, db_1.query)(`INSERT INTO users (company_id, login_id, email, password_hash, role, must_change_password)
     VALUES ($1, $2, 'john.doe@nmitodoo.com', $3, 'employee', TRUE) RETURNING user_id`, [companyId, emp1LoginId, await bcryptjs_1.default.hash(tempPass, 12)]);
    const employeeUserId = emp1Res.rows[0].user_id;
    // Insert profile
    await (0, db_1.query)(`INSERT INTO employee_profiles (user_id, company_id, first_name, last_name, joining_serial_no)
     VALUES ($1, $2, 'John', 'Doe', $3)`, [employeeUserId, companyId, emp1Serial]);
    // 3. Attendance Check In/Out
    const today = new Date().toISOString().split('T')[0];
    const checkInRes = await (0, db_1.query)(`INSERT INTO attendance (user_id, date, check_in_time)
     VALUES ($1, $2, NOW()) RETURNING *`, [employeeUserId, today]);
    ...
    await (0, db_1.query)(`UPDATE attendance
     ...
     WHERE user_id = $3 AND date = $4`, [simulatedCheckIn, simulatedCheckOut, employeeUserId, today]);
    const workHrsRes = await (0, db_1.query)('SELECT work_hours FROM attendance WHERE user_id = $1 AND date = $2', [
        employeeUserId,
        today,
    ]);
    ...
    // Create leave type
    const ltRes = await (0, db_1.query)(`INSERT INTO leave_types (company_id, name, max_days_per_year, is_paid)
     VALUES ($1, 'Paid Time Off', 24, TRUE) RETURNING leave_type_id`, [companyId]);
    const leaveTypeId = ltRes.rows[0].leave_type_id;
    // Balance init
    await (0, db_1.query)(`INSERT INTO leave_balances (user_id, leave_type_id, year, total_days, used_days)
     VALUES ($1, $2, 2026, 24, 0)`, [employeeUserId, leaveTypeId]);
    // Apply request
    const reqRes = await (0, db_1.query)(`INSERT INTO leave_requests (user_id, leave_type_id, start_date, end_date, total_days, status)
     VALUES ($1, $2, '2026-10-10', '2026-10-12', 3, 'pending') RETURNING leave_request_id`, [employeeUserId, leaveTypeId]);
    const requestId = reqRes.rows[0].leave_request_id;
    await assert(!!requestId, 'Leave request submitted successfully');
    // Review (Approve)
    await (0, db_1.query)(`UPDATE leave_requests SET status = 'approved', reviewed_by = $1 WHERE leave_request_id = $2`, [adminUserId, requestId]);
    // Update used days on balance
    await (0, db_1.query)(`UPDATE leave_balances SET used_days = used_days + 3 WHERE user_id = $1 AND leave_type_id = $2 AND year = 2026`, [employeeUserId, leaveTypeId]);
    const balRes = await (0, db_1.query)('SELECT remaining_days FROM leave_balances WHERE user_id = $1', [employeeUserId]);
    ...
    // Save structure
    const structRes = await (0, db_1.query)(`INSERT INTO salary_structures (user_id, month_wage, pf_employee_rate, pf_employer_rate, professional_tax, effective_from, created_by)
     VALUES ($1, $2, 12, 12, 200, '2026-01-01', $3) RETURNING salary_id`, [employeeUserId, monthWage, adminUserId]);
    const salaryId = structRes.rows[0].salary_id;
    // Insert components
    await (0, db_1.query)(`INSERT INTO salary_components (salary_id, component_name, computation_type, computed_amount) VALUES
     ...
     ($1, 'Fixed Allowance', 'fixed_amount', $6)`, [salaryId, basic, hra, stdAllow, bonus, expectedFixed]);
    const sumRes = await (0, db_1.query)('SELECT SUM(computed_amount) FROM salary_components WHERE salary_id = $1', [salaryId]);
