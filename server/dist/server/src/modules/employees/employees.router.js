...
const createEmployeeSchema = zod_1.z.object({
    body: zod_1.z.object({
        ...
        dateOfJoining: zod_1.z.string().optional(), // YYYY-MM-DD
    }),
});
router.post('/', auth_1.authenticate, (0, auth_1.requireRole)(['admin']), (0, validate_1.validate)(createEmployeeSchema), async (req, res) => {
    try {
        const { firstName, lastName, email, phone, departmentId, managerId, designation, location, dateOfJoining, } = req.body;
        const companyId = req.user.companyId;
        // Check existing email
        const existing = await (0, db_1.query)('SELECT user_id FROM users WHERE email = $1', [email]);
        if (existing.rows.length > 0) {
            return res.status(400).json({ error: 'Email already registered' });
        }
        // Fetch company name
        const compRes = await (0, db_1.query)('SELECT name FROM companies WHERE company_id = $1', [companyId]);
        ...
        const { loginId, serialNo } = await (0, loginId_1.generateLoginId)(companyId, companyName, firstName, lastName, joiningYear);
        ...
        const userRes = await (0, db_1.query)(`INSERT INTO users (company_id, login_id, email, phone, password_hash, role, must_change_password, is_email_verified)
         ...
         RETURNING user_id, company_id, login_id, email, role, must_change_password`, [companyId, loginId, email, phone || null, passwordHash]);
        const newUser = userRes.rows[0];
        // Create profile
        const profRes = await (0, db_1.query)(`INSERT INTO employee_profiles
        ...
        await (0, db_1.query)('INSERT INTO employee_resume (user_id) VALUES ($1)', [newUser.user_id]);
        ...
        const leaveTypes = await (0, db_1.query)('SELECT leave_type_id, max_days_per_year FROM leave_types WHERE company_id = $1', [
            companyId,
        ]);
        for (const lt of leaveTypes.rows) {
           ...
           VALUES ($1, $2, $3, $4, 0)
           ON CONFLICT (user_id, leave_type_id, year) DO NOTHING`, [newUser.user_id, lt.leave_type_id, joiningYear, lt.max_days_per_year]);
        }
        // Record audit log
        await (0, db_1.query)(`INSERT INTO audit_logs (user_id, action, entity_type, entity_id, metadata)
         VALUES ($1, 'CREATE_EMPLOYEE', 'users', $2, $3)`, [req.user.userId, newUser.user_id, JSON.stringify({ loginId, email })]);
...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const employees = await (0, db_1.query)(`SELECT 
        u.user_id,
        u.login_id,
        u.email,
        u.phone,
       ...
       ORDER BY p.first_name ASC`, [companyId, `%${search}%`]);
...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const emp = await (0, db_1.query)(`SELECT 
       ...
       LEFT JOIN departments d ON d.department_id = p.department_id
       WHERE u.user_id = $1 AND u.company_id = $2`, [targetUserId, companyId]);
