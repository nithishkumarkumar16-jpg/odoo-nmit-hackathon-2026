...
router.get('/structure/:userId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const compRes = await query(
      `SELECT * FROM salary_components WHERE salary_id = $1 ORDER BY display_order ASC`,
      [structure.salary_id]
    );
...
router.post('/structure', authenticate, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const {
      userId,
      monthWage,
      ...
      pfEmployerRate,
      professionalTax,
      components, // Array of { componentName, computationType, rateValue, fixedAmount }
    } = req.body;
    ...
    const basicComp = inputComponents.find((c: any) => c.componentName === 'Basic Salary');
    if (basicComp) {
    ...
    for (const comp of inputComponents) {
      if (comp.componentName === 'Fixed Allowance') continue; // Calculate remainder later
    ...
    for (const c of computedList) {
      await query(
        `INSERT INTO salary_components
         ...
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [salaryId, c.componentName, c.computationType, c.rateValue, c.computedAmount, c.displayOrder]
      );
    }
...
router.post('/generate', authenticate, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const employees = await query('SELECT user_id FROM users WHERE company_id = $1 AND is_active = TRUE', [
      companyId,
    ]);
    ...
    for (const emp of employees.rows) {
      ...
      const salRes = await query(
        `SELECT * FROM salary_structures WHERE user_id = $1 AND effective_from <= $2 ORDER BY effective_from DESC LIMIT 1`,
        [userId, endDate]
      );

      if (salRes.rows.length === 0) continue; // Skip if no salary structure set
      const sal = salRes.rows[0];

      // 2. Compute attendance present days
      const attRes = await query(
        `SELECT COUNT(DISTINCT date) as present_days FROM attendance WHERE user_id = $1 AND date >= $2 AND date <= $3 AND check_in_time IS NOT NULL`,
      ...
      const leaveRes = await query(
        `SELECT lr.total_days, lt.is_paid
      ...
      for (const l of leaveRes.rows) {
        if (l.is_paid) paidLeaveDays += Number(l.total_days);
        else unpaidLeaveDays += Number(l.total_days);
      }
...
router.get('/payslips', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const currentUserId = req.user!.userId;
    const role = req.user!.role;
    const companyId = req.user!.companyId;

    const targetUserId = req.query.userId ? (req.query.userId as string) : currentUserId;
    ...
    const slips = await query(
      `SELECT ps.*, p.first_name, p.last_name, u.login_id
       FROM payslips ps
      ...
      role !== 'admin' || req.query.userId ? [companyId, targetUserId] : [companyId]
    );
