...
router.get('/status', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const attRes = await query(
      `SELECT attendance_id, date, check_in_time, check_out_time, work_hours
       FROM attendance
...
router.post('/check-in', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await query('SELECT * FROM attendance WHERE user_id = $1 AND date = $2', [
      userId,
      today,
    ]);
    ...
    if (existing.rows.length > 0) {
      const updated = await query(
        `UPDATE attendance SET check_in_time = NOW() WHERE user_id = $1 AND date = $2 RETURNING *`,
        [userId, today]
      );
      attRecord = updated.rows[0];
    } else {
      const inserted = await query(
        `INSERT INTO attendance (user_id, date, check_in_time) VALUES ($1, $2, NOW()) RETURNING *`,
        [userId, today]
      );
...
router.post('/check-out', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await query(
      `UPDATE attendance SET check_out_time = NOW() WHERE user_id = $1 AND date = $2 RETURNING *`,
      [userId, today]
    );
...
router.get('/me', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const salRes = await query(
      `SELECT standard_shift_hours FROM salary_structures WHERE user_id = $1 ORDER BY effective_from DESC LIMIT 1`,
      [userId]
    );
    ...
    const records = await query(
      `SELECT 
       ...
       ORDER BY a.date ASC`,
      [userId, startDate, standardShift, endDate]
    );

    // Header summary stats
    const presentCount = records.rows.filter((r: any) => r.check_in_time).length;

    const leaveCountRes = await query(
    ...
    const totalWorkHours = records.rows.reduce(
      (sum: number, r: any) => sum + (Number(r.work_hours) || 0),
      0
    );
